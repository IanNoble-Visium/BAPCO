...
function simulateRealTimeData() {
    ...
    MockData.kpis.production.value += Math.round((Math.random() - 0.5) * 500);
    MockData.kpis.efficiency.value = parseFloat((MockData.kpis.efficiency.value + (Math.random() - 0.5) * 0.2).toFixed(1));
    MockData.kpis.energy.value += Math.round((Math.random() - 0.5) * 20);
    MockData.kpis.water.value += Math.round((Math.random() - 0.5) * 50);

    // Update unit temperatures
    MockData.units.forEach(unit => {
        unit.temperature += (Math.random() - 0.5) * 2;
        unit.throughput += Math.round((Math.random() - 0.5) * 100);
    });

    // Update tank levels
    MockData.tanks.forEach(tank => {
        tank.level = Math.max(20, Math.min(95, tank.level + (Math.random() - 0.5) * 0.5));
    });
